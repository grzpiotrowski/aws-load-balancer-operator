...
if [[ -z "${OPERATOR_IMAGE_PULLSPEC}" || -z "${OPERAND_IMAGE_PULLSPEC}" ]]; then
  ...
  echo "Example:"
  echo "  OPERATOR_IMAGE_PULLSPEC=quay.io/openshift/aws-load-balancer-operator@sha256:<digest> \\\n  OPERAND_IMAGE_PULLSPEC=quay.io/openshift/origin-aws-load-balancer-controller@sha256:<digest>"
  exit 2
fi
...
# Add OCP annotations
if [[ -n "${SUPPORTED_OCP_VERSIONS}" ]]; then
python3 - << END
import os
from ruamel.yaml import YAML
yaml = YAML()
...
with open(os.getenv('METADATA_DIR') + "/annotations.yaml", 'w') as f:
    yaml.dump(y, f)
END
[ $? -ne 0 ] && { echo "ERROR: Error rendering metadata template."; exit 7; }
fi
